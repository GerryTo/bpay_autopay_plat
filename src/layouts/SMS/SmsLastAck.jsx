...
const SmsLastAck = () => {
  ...
  const fetchData = async ({ silent = false } = {}) => {
    ...
    try {
      const response = await smsAPI.getLastAckList();
      ...
      if (response.success && response.data) {
        if ((response.data.status || '').toLowerCase() === 'ok') {
          ...
          showNotification({
            title: 'Error',
            message:
              response.data.message || 'Failed to load SMS last ACK data',
            color: 'red',
          });
        }
      } else {
        showNotification({
          title: 'Error',
          message: response.error || 'Failed to load SMS last ACK data',
          color: 'red',
        });
      }
    } catch (error) {
      console.error('SMS last ACK fetch error:', error);
      showNotification({
        title: 'Error',
        message: 'Unable to load SMS last ACK data',
        color: 'red',
      });
    } finally {
      setLoading(false);
          ...
          >
            <Stack gap={4}>
              ...
              <Text
                ...
                c="dimmed"
              >
                Review the latest ACK timestamps for SMS devices. Totals are
                shown in the footer.
              </Text>
            </Stack>
