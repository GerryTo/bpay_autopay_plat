...
const SmsLogHistory = () => {
  ...
  const columns = useMemo(
    () => [
      ...
      },
      {
        ...
        minWidth: 130,
        render: (item) => {
          const isExpired = item.futuretrxid === '-1' || item.futuretrxid === -1;
          return (
            <Text size="sm" fw={600} c={isExpired ? 'red' : undefined}>
        ...
        render: (item) => (
          <Group gap={6} wrap="wrap">
            <Button
              ...
              variant="light"
              color="blue"
              disabled={!!item.futuretrxid && item.futuretrxid !== '' && item.futuretrxid !== '-1'}
  ...
  useEffect(() => {
    const fetchPhoneUsers = async () => {
      ...
      if (response.success && response.data?.status?.toLowerCase?.() === 'ok') {
        setPhoneUsers(Array.isArray(response.data.records) ? response.data.records : []);
      }
    };
    fetchPhoneUsers();
  }, []);
  ...
  const fetchData = async ({ silent = false } = {}) => {
    ...
    try {
      const response = await smsAPI.getLogHistory({
        datefrom: from,
        dateto: to,
        type,
        user,
      ...
      if (response.success && response.data) {
        if ((response.data.status || '').toLowerCase() === 'ok') {
          ...
          showNotification({
            title: 'Error',
            message: response.data.message || 'Failed to load SMS log history data',
            color: 'red',
          });
        }
      } else {
        showNotification({
          title: 'Error',
          message: response.error || 'Failed to load SMS log history data',
          color: 'red',
        });
      }
    } catch (error) {
      console.error('SMS log history fetch error:', error);
      showNotification({
        title: 'Error',
        message: 'Unable to load SMS log history data',
        color: 'red',
      });
    } finally {
      setLoading(false);
  ...
  const handleExpire = async (item) => {
    ...
    setRefreshing(true);
    try {
      const response = await smsAPI.expireSmsHistory({
        amount: item.amount,
        bank: item.type,
  ...
  const handleMatch = async (item) => {
    ...
    setRefreshing(true);
    try {
      const response = await smsAPI.matchSmsHistory({
        futuretrxid,
        amount: item.amount,
        bank: item.type,
        trxid: item.securitycode,
          ...
          <Group justify="space-between" align="flex-start">
            <Stack gap={4}>
              ...
              <Text size="sm" c="dimmed">
                View historical SMS logs by date range, type, and phone user. Totals are shown in the footer.
              </Text>
            </Stack>
