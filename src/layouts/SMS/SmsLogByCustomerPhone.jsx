...
const SmsLogByCustomerPhone = () => {
  ...
  const fetchData = async ({ silent = false } = {}) => {
    if (!customerPhone.trim()) {
      showNotification({
        title: 'Validation',
        message: 'Please input customer phone first',
        color: 'yellow',
      });
      return;
    }
    ...
    try {
      const response = await smsAPI.getLogByCustomerPhone({
      ...
      console.error('SMS log by customer phone fetch error:', error);
      showNotification({
        title: 'Error',
          ...
          <Group justify="space-between" align="flex-start">
            <Stack gap={4}>
              <Group gap={8}>
                <IconMessage size={22} />
                <Text size="xl" fw={700}>
                  SMS Log by Customer Phone
                </Text>
              </Group>
              <Text size="sm" c="dimmed">
                Search SMS logs using a customer phone number. Totals are in the footer.
              </Text>
            </Stack>
...
export default SmsLogByCustomerPhone;
