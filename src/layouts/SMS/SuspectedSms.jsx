...
const SuspectedSms = () => {
    ...
    () =>
      data.filter((item) => {
        return (
          ...
          includesValue(item.balancecalculate, columnFilters.balancecalculate) &&
  ...
  const fetchData = async ({ silent = false } = {}) => {
    ...
    try {
      const response = await smsAPI.getSuspectedSmsList({
        datefrom: from,
        dateto: to,
        user,
      });
      ...
      if (response.success && response.data) {
        if ((response.data.status || '').toLowerCase() === 'ok') {
          ...
          showNotification({
            title: 'Error',
            message: response.data.message || 'Failed to load suspected SMS data',
            color: 'red',
          });
        }
      } else {
        showNotification({
          title: 'Error',
          message: response.error || 'Failed to load suspected SMS data',
          color: 'red',
        });
      }
    } catch (error) {
      console.error('Suspected SMS fetch error:', error);
      showNotification({
        title: 'Error',
        message: 'Unable to load suspected SMS data',
        color: 'red',
      });
    } finally {
      setLoading(false);
          ...
          <Group justify="space-between" align="flex-start">
            <Stack gap={4}>
              ...
              <Text size="sm" c="dimmed">
                Review suspected SMS by date range and phone user. Totals sit in the footer.
              </Text>
            </Stack>
