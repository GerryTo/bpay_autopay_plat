...
const SmsFailedMatchByNotMatchSameday = () => {
  ...
  const fetchData = async ({ silent = false } = {}) => {
    ...
    try {
      const response = await smsAPI.getFailedMatchByNotMatchSameday({
        datefrom: from,
        dateto: to,
      });
      ...
      console.error('SMS failed match by not match sameday fetch error:', error);
      showNotification({
        title: 'Error',
          ...
          <Group justify="space-between" align="flex-start">
            <Stack gap={4}>
              <Group gap={8}>
                <IconMessage size={22} />
                <Text size="xl" fw={700}>
                  SMS Failed Match (Not Match Sameday)
                </Text>
              </Group>
              <Text size="sm" c="dimmed">
                Review SMS failed matches filtered to not-match-sameday cases. Totals sit in the footer.
              </Text>
            </Stack>
...
export default SmsFailedMatchByNotMatchSameday;
