...
const ReportSms = () => {
  ...
  const columns = useMemo(
    () => [
      ...
      },
      {
        ...
        label: 'No. of SMS',
        minWidth: 140,
        render: (item) => <Text size="sm">{item.noSms ?? '-'}</Text>,
        filter: (
          <TextInput
            placeholder="Filter count..."
            size="xs"
            value={columnFilters.noSms}
            onChange={(e) => handleFilterChange('noSms', e.currentTarget.value)}
          />
        ),
      },
      {
    ...
    () =>
      data.filter((item) => {
        return (
          ...
          includesValue(item.noSms, columnFilters.noSms) &&
  ...
  const fetchData = async ({ silent = false } = {}) => {
    ...
    try {
      const response = await smsAPI.getReportSms({
        datefrom: from,
        dateto: to,
      });
      ...
      if (response.success && response.data) {
        if ((response.data.status || '').toLowerCase() === 'ok') {
          ...
          showNotification({
            title: 'Error',
            message: response.data.message || 'Failed to load SMS report data',
            color: 'red',
          });
        }
      } else {
        showNotification({
          title: 'Error',
          message: response.error || 'Failed to load SMS report data',
          color: 'red',
        });
      }
    } catch (error) {
      console.error('SMS report fetch error:', error);
      showNotification({
        title: 'Error',
        message: 'Unable to load SMS report data',
        color: 'red',
      });
    } finally {
      setLoading(false);
    ...
    () =>
      filteredData.reduce(
        (acc, item) => {
          ...
          acc.noSms += Number(item.noSms) || 0;
          return acc;
        },
        { rows: 0, amount: 0, noSms: 0 }
      ),
    [filteredData]
  );
          ...
          >
            <Stack gap={4}>
              ...
              <Text
                ...
                c="dimmed"
              >
                View SMS summaries by phone number. Totals are shown in the
                footer.
              </Text>
            </Stack>
