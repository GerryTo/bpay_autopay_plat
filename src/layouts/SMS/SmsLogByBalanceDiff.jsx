...
const SmsLogByBalanceDiff = () => {
  ...
  const fetchData = async ({ silent = false } = {}) => {
    if (!diffType) {
      showNotification({
        title: 'Validation',
        message: 'Please select balance diff option',
        color: 'yellow',
      });
      return;
    }
    ...
    try {
      const response = await smsAPI.getLogByBalanceDiff({
        type: diffType,
      });
      ...
      console.error('SMS log by balance diff fetch error:', error);
      showNotification({
        title: 'Error',
          ...
          >
            <Stack gap={4}>
              ...
              <Text
                ...
                c="dimmed"
              >
                View SMS logs filtered by balance difference. Totals sit in the
                footer.
              </Text>
            </Stack>
          ...
          >
            ...
            >
              <Select
                ...
                data={balanceDiffOptions}
                value={diffType}
                onChange={(value) => setDiffType(value || '0')}
                placeholder="Select diff range"
                style={{ minWidth: 200 }}
              />
              {/* <Group gap="xs">
                <Button
...
export default SmsLogByBalanceDiff;
