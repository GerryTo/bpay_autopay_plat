...
const SmsLogBackup = () => {
  ...
  useEffect(() => {
    const fetchPhoneUsers = async () => {
      const response = await smsAPI.getBackupPhoneUserList();
  ...
  const fetchData = async ({ silent = false } = {}) => {
    ...
    try {
      const response = await smsAPI.getLogBackupList({
        datefrom: from,
        dateto: to,
        type,
        user,
      ...
      if (response.success && response.data) {
        if ((response.data.status || '').toLowerCase() === 'ok') {
          ...
          showNotification({
            title: 'Error',
            message:
              response.data.message || 'Failed to load SMS log backup data',
            color: 'red',
          });
        }
      } else {
        showNotification({
          title: 'Error',
          message: response.error || 'Failed to load SMS log backup data',
          color: 'red',
        });
      }
    } catch (error) {
      console.error('SMS log backup fetch error:', error);
      showNotification({
        title: 'Error',
        message: 'Unable to load SMS log backup data',
        color: 'red',
      });
    } finally {
      setLoading(false);
          ...
          >
            <Stack gap={4}>
              ...
              <Text
                ...
                c="dimmed"
              >
                View historical SMS logs by date range, status, and phone user.
