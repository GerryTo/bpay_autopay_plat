  ...
  const fetchList = useCallback(
    async ({ silent = false } = {}) => {
      ...
      try {
        ...
        const response = await withdrawAPI.getWithdrawNtc({
        ...
        if (response.success && response.data) {
          ...
          if ((payload.status || '').toLowerCase() === 'ok') {
            ...
            showNotification({
              title: 'Error',
              message: payload.message || 'Failed to load withdraw check list',
              color: 'red',
            });
          }
        } else {
          showNotification({
            title: 'Error',
            message: response.error || 'Failed to load withdraw check list',
            color: 'red',
          });
        }
      } catch (error) {
        console.error('Withdraw check list fetch error:', error);
        showNotification({
          title: 'Error',
          message: 'Unable to load withdraw check list',
          color: 'red',
        });
      } finally {
        setLoading(false);
