...
const RequestList = () => {
  ...
  const fetchList = useCallback(async ({ silent = false } = {}) => {
    ...
    try {
      const response = await crawlerAPI.getRequestList();
      ...
      if (response.success && response.data) {
        ...
        if ((payload.status || '').toLowerCase() === 'ok') {
          ...
          showNotification({
            title: 'Error',
            message: payload.message || 'Failed to load request list',
            color: 'red',
          });
        }
      } else {
        showNotification({
          title: 'Error',
          message: response.error || 'Failed to load request list',
          color: 'red',
        });
      }
    } catch (error) {
      console.error('Request list fetch error:', error);
      showNotification({
        title: 'Error',
        message: 'Unable to load request list',
        color: 'red',
      });
    } finally {
      setLoading(false);
