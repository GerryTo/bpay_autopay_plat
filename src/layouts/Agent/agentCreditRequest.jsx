...
const AgentCreditRequest = () => {
  ...
  const getListData = async () => {
    ...
    try {
      ...
      const response = await agentCommissionAPI.getCreditRequestList(from, to);
  ...
  const handleApprove = async (request) => {
    if (window.confirm('Are you sure want to approve this request?')) {
      try {
        const response = await agentCommissionAPI.approveCreditRequest(
          request.id
        );
        ...
        if (response.success && response.data) {
          ...
          showNotification({
            title: 'Error',
            message: response.error || 'Failed to approve request',
            color: 'red',
          });
        }
      } catch (error) {
  ...
  const handleRejectConfirm = async () => {
    ...
    try {
      const response = await agentCommissionAPI.rejectCreditRequest(
        selectedRequest.id,
        rejectReason
      );
      ...
      if (response.success && response.data) {
        ...
        showNotification({
          title: 'Error',
          message: response.error || 'Failed to reject request',
          color: 'red',
        });
      }
    } catch (error) {
  ...
  const rows = data.map((item, index) => (
    <Table.Tr key={index}>
      ...
      <Table.Td>
        <Text size="sm">{item.bankaccountno}</Text>
      </Table.Td>
      <Table.Td>
          ...
          >
            ...
            >
              ...
              >
                <Table.Thead>
                  <Table.Tr>
                    <Table.Th style={{ minWidth: 120 }}>Request ID</Table.Th>
