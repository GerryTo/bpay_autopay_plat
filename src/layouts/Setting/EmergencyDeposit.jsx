...
import { IconCalendar, IconFilter, IconRefresh, IconAlertTriangle } from '@tabler/icons-react';
import { emergencyDepositAPI } from '../../helper/api';
...
const mapRecords = (records = []) =>
  records.map((item, idx) => {
    ...
    return {
      ...item,
      _rowKey: `${item.futuretrxid || 'row'}-${idx}`,
      DB: isDebit ? amount : 0,
      CR: isDebit ? 0 : amount,
...
const EmergencyDeposit = () => {
    ...
    () =>
      data.filter((item) => {
        return (
          ...
          includesValue(item.callbackresponse, columnFilters.callbackresponse) &&
  ...
  const fetchList = useCallback(
    async ({ silent = false } = {}) => {
      ...
      const diffDays = dayjs(end).startOf('day').diff(dayjs(start).startOf('day'), 'day');
      if (diffDays > 6) {
        showNotification({
          title: 'Validation',
      ...
      try {
        ...
        const response = await emergencyDepositAPI.getList({
        ...
        if (response.success && response.data) {
          ...
          if (!payload.status || status === 'ok') {
            ...
            showNotification({
              title: 'Error',
              message: payload.message || 'Failed to load emergency deposits',
              color: 'red',
            });
          }
        } else {
          showNotification({
            title: 'Error',
            message: response.error || 'Failed to load emergency deposits',
            color: 'red',
          });
        }
      } catch (error) {
        console.error('Emergency deposit fetch error:', error);
        showNotification({
          title: 'Error',
          message: 'Unable to load emergency deposits',
          color: 'red',
        });
      } finally {
        setLoading(false);
  ...
  const runEmergency = async () => {
    ...
    const confirmed = window.confirm('Run emergency process for selected date range?');
    if (!confirmed) return;
    ...
    try {
      const payloadDateFrom = dayjs(start).format('YYYY-MM-DD');
      const payloadDateTo = dayjs(end).format('YYYY-MM-DD');
      const response = await emergencyDepositAPI.runEmergency({
        dateFrom: payloadDateFrom,
        dateTo: payloadDateTo,
      });
      if (response.success && response.data) {
        ...
        showNotification({
          ...
          message: response.data.message || 'Emergency process triggered',
        ...
        showNotification({
          title: 'Error',
          message: response.error || 'Failed to trigger emergency process',
          color: 'red',
        });
      }
    } catch (error) {
      console.error('Emergency run error:', error);
      showNotification({
        title: 'Error',
        message: 'Unable to run emergency process',
        color: 'red',
      });
    } finally {
      setLoading(false);
          ...
          >
            <Box>
              ...
              <Text
                ...
                c="dimmed"
              >
                Monitor and trigger emergency deposit process.
              </Text>
            </Box>
                  ...
                  <Table.Tr>
                    <Table.Td colSpan={visibleColumns.length}>
                      ...
                      >
                        ...
                        <Group gap="md">
                          <Text size="sm">Debit: {formatNumber(totalDebit)}</Text>
                          <Text size="sm">Credit: {formatNumber(totalCredit)}</Text>
                          <Text size="sm">Fee: {formatNumber(totalFee)}</Text>
                        </Group>
                      </Group>
                    </Table.Td>
                  </Table.Tr>
...
export default EmergencyDeposit;
