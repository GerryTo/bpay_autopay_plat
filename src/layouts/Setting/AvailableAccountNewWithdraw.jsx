...
import { availableAccountNewWdAPI } from '../../helper/api';
...
const AvailableAccountNewWithdraw = () => {
  ...
  const makeKey = (item) =>
    item._rowKey ||
    `${item.v_bankcode || 'bank'}-${item.v_bankaccountno || 'acct'}-${
      item.v_user || 'user'
    }`;
    ...
    () =>
      data.filter((item) => {
        return (
          ...
          includesValue(item.n_lastBalance, columnFilters.lastBalance)
        );
      }),
    [data, columnFilters]
  );
    ...
    () => [
      ...
      },
      {
        ...
        render: (item) => (
          <Badge
            variant="light"
            color={
              (item.v_isactive || '').toString().toLowerCase() === 'y'
                ? 'green'
                : 'gray'
            }
          >
      ...
      },
      {
        ...
        filter: (
          <TextInput
            ...
            onChange={(e) =>
              handleFilterChange('useCredit', e.currentTarget.value)
            }
          />
        ),
      },
      {
        ...
        render: (item) => (
          ...
          >
            {formatNumber(item.n_lastBalance)}
          </Text>
        ),
        filter: (
          <TextInput
            ...
            size="xs"
            value={columnFilters.lastBalance}
            onChange={(e) =>
              handleFilterChange('lastBalance', e.currentTarget.value)
            }
          />
        ),
      },
  ...
  const fetchList = useCallback(async ({ silent = false } = {}) => {
    ...
    try {
      const response = await availableAccountNewWdAPI.getList();
      if (response.success && response.data) {
        ...
        if (!payload.status || status === 'success' || status === 'ok') {
          ...
          const withKeys = records.map((item, idx) => ({
            ...item,
            _rowKey: `${item.v_bankcode || 'bank'}-${
              item.v_bankaccountno || 'acct'
            }-${item.v_user || 'user'}-${idx}`,
          }));
          setData(withKeys);
        } else {
          showNotification({
      ...
      console.error('Available account new withdraw fetch error:', error);
      showNotification({
        title: 'Error',
  ...
  const handleSubmitBulk = async () => {
    ...
    try {
      ...
      const response = await availableAccountNewWdAPI.updateSelected(payload);
      if (response.success && response.data) {
  ...
  const handlePriority = async () => {
    ...
    // Unique by user
    const seen = new Set();
    const uniqueByUser = selectedItems.filter((item) => {
      const u = item.v_user || '';
    ...
    setSaving(true);
    try {
      const payload = { group: 5, list: uniqueByUser };
      const response = await availableAccountNewWdAPI.updateSelected(payload);
      if (response.success && response.data) {
        const status = (response.data.status || '').toLowerCase();
        const message = response.data.message || 'Priority update submitted';
        showNotification({
          title: status === 'ok' ? 'Success' : 'Info',
          message,
          color: status === 'ok' ? 'green' : 'blue',
        ...
        showNotification({
          title: 'Error',
          message: response.error || 'Failed to update priority',
          color: 'red',
        });
      }
    } catch (error) {
  ...
  const handleRerun = async () => {
    const confirmed = window.confirm(
      'Re-run available account new withdraw process?'
    );
    if (!confirmed) return;
    setSaving(true);
    try {
      const response = await availableAccountNewWdAPI.rerun();
      const payload = response.data || {};
      const status = (payload.status || '').toLowerCase();
      showNotification({
        title: status === 'ok' ? 'Success' : 'Info',
      ...
      console.error('Rerun withdraw error:', error);
      showNotification({
        title: 'Error',
  ...
  const handleSaveEdit = async () => {
    ...
    try {
      ...
      const response = await availableAccountNewWdAPI.updateSingleGroup(
        payload
      );
      if (response.success && response.data) {
        const status = (response.data.status || '').toLowerCase();
      ...
      console.error('Single edit withdraw error:', error);
      showNotification({
        title: 'Error',
          ...
          >
            <Box>
              ...
              <Text
                ...
                c="dimmed"
              >
                Manage withdraw account grouping and priority.
              </Text>
            </Box>
...
export default AvailableAccountNewWithdraw;
