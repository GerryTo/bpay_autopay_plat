...
import { countAvailableAccountAPI, availableAccountNewAPI } from '../../helper/api';
...
const CountAvailableAccountNew = () => {
  ...
  const makeKey = (item) => `${item.v_bankcode || ''}-${item.v_bankaccountno || ''}-${item.v_user || ''}`;
  ...
  const fetchList = useCallback(
    async ({ silent = false } = {}) => {
      ...
      try {
        const response = await countAvailableAccountAPI.getList();
        if (response.success && response.data) {
          ...
          if (!payload.status || status === 'success' || status === 'ok') {
            ...
            showNotification({
              title: 'Error',
              message: payload.message || 'Failed to load accounts',
              color: 'red',
            });
          }
        } else {
        ...
        console.error('Count available account fetch error:', error);
        showNotification({
          title: 'Error',
          ...
          >
            <Box>
              <Text
                ...
                fw={700}
              >
                Count Available Account New Deposit (With Mybank)
              </Text>
              <Text
                size="sm"
                c="dimmed"
              >
                View and update account grouping with Mybank data.
              </Text>
            </Box>
...
export default CountAvailableAccountNew;
