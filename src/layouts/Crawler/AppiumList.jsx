  ...
  const columns = useMemo(
    () => [
      ...
      },
      {
        ...
        minWidth: 160,
        render: (item) => (
          <Text size="sm">{formatFutureTrx(item.futuretrxid)}</Text>
        ),
        filter: (
          <TextInput
  ...
  const fetchList = useCallback(
    async ({ silent = false } = {}) => {
      ...
      try {
        ...
        const response = await crawlerAPI.getAppiumList({
        ...
        if (response.success && response.data) {
          ...
          if ((payload.status || '').toLowerCase() === 'ok') {
            ...
            showNotification({
              title: 'Error',
              message: payload.message || 'Failed to load crawler list',
              color: 'red',
            });
          }
        } else {
          showNotification({
            title: 'Error',
            message: response.error || 'Failed to load crawler list',
            color: 'red',
          });
        }
      } catch (error) {
        console.error('Crawler list fetch error:', error);
        showNotification({
          title: 'Error',
          message: 'Unable to load crawler list',
          color: 'red',
        });
      } finally {
        setLoading(false);
                ...
                <Table.Tbody>
                  {paginatedData.length > 0 ? (
                    paginatedData.map((item, idx) => (
                      <Table.Tr
                        key={`${item.futuretrxid || item.trxid || 'row'}-${
                          startIndex + idx
                        }`}
                      >
                        {visibleColumns.map((col) => (
