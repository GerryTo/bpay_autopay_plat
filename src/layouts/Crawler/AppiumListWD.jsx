      ...
      },
      {
        ...
        filter: (
          <TextInput
            ...
            onChange={(e) => handleFilterChange('account', e.currentTarget.value)}
          />
        ),
      },
      {
      ...
      try {
        ...
        if (response.success && response.data) {
          ...
          if ((payload.status || '').toLowerCase() === 'ok') {
            ...
            showNotification({
              title: 'Error',
              message: payload.message || 'Failed to load crawler list WD',
              color: 'red',
            });
          }
        } else {
          showNotification({
            title: 'Error',
            message: response.error || 'Failed to load crawler list WD',
            color: 'red',
          });
        }
      } catch (error) {
        console.error('Crawler list WD fetch error:', error);
        showNotification({
          title: 'Error',
          message: 'Unable to load crawler list WD',
          color: 'red',
        });
      } finally {
        setLoading(false);
          ...
          >
            <Box>
              ...
              <Text
                ...
                c="dimmed"
              >
                Appium crawler withdrawal records
              </Text>
            </Box>
                ...
                <Table.Tbody>
                  {paginatedData.length > 0 ? (
                    paginatedData.map((item, idx) => (
                      <Table.Tr key={`${item.trxid || 'row'}-${startIndex + idx}`}>
                        {visibleColumns.map((col) => (
