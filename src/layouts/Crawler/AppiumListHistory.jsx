  ...
  const fetchList = useCallback(
    async ({ silent = false } = {}) => {
      ...
      try {
        ...
        const response = await crawlerAPI.getAppiumListHistory({
        ...
        if (response.success && response.data) {
          ...
          if ((payload.status || '').toLowerCase() === 'ok') {
            ...
            showNotification({
              title: 'Error',
              message: payload.message || 'Failed to load crawler history',
              color: 'red',
            });
          }
        } else {
          showNotification({
            title: 'Error',
            message: response.error || 'Failed to load crawler history',
            color: 'red',
          });
        }
      } catch (error) {
        console.error('Crawler history fetch error:', error);
        showNotification({
          title: 'Error',
          message: 'Unable to load crawler history',
          color: 'red',
        });
      } finally {
        setLoading(false);
          ...
          >
            <Box>
              <Text
                ...
                fw={700}
              >
                Crawler List History
              </Text>
              <Text
                size="sm"
                c="dimmed"
              >
                Appium crawler history records
              </Text>
            </Box>
...
export default AppiumListHistory;
