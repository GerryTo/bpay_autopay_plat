...
const formatFutureTrx = (value) => {
  const num = Number(value);
  if (!Number.isNaN(num) && num === -1) return 'Expired';
  if (value === null || value === undefined || value === '') return '-';
  return value;
};

const AppiumListNotMatch = () => {
  ...
  const columns = useMemo(
    () => [
      ...
      {
        key: 'dst_account_name',
        label: 'Account',
        minWidth: 180,
        render: (item) => <Text size="sm">{item.dst_account_name || '-'}</Text>,
        filter: (
          <TextInput
            placeholder="Filter account..."
            size="xs"
            value={columnFilters.dst_account_name}
            onChange={(e) => handleFilterChange('dst_account_name', e.currentTarget.value)}
          />
        ),
      },
      {
        key: 'dst_bank_account_no',
        label: 'Phone Number',
        minWidth: 160,
        render: (item) => <Text size="sm">{item.dst_bank_account_no || '-'}</Text>,
        filter: (
          <TextInput
            placeholder="Filter phone..."
            size="xs"
            value={columnFilters.dst_bank_account_no}
            onChange={(e) => handleFilterChange('dst_bank_account_no', e.currentTarget.value)}
          />
        ),
      },
      {
      ...
      },
      {
        ...
        label: 'Future Trx ID',
        minWidth: 160,
        render: (item) => <Text size="sm">{formatFutureTrx(item.futuretrxid)}</Text>,
        filter: (
          <TextInput
    ...
    () =>
      data.filter((item) => {
        return (
          ...
          includesValue(item.dst_account_name, columnFilters.dst_account_name) &&
          includesValue(item.dst_bank_account_no, columnFilters.dst_bank_account_no) &&
  ...
  const fetchList = useCallback(
    async ({ silent = false } = {}) => {
      ...
      try {
        ...
        const response = await crawlerAPI.getAppiumListNotMatch({
        ...
        if (response.success && response.data) {
          ...
          if ((payload.status || '').toLowerCase() === 'ok') {
            ...
            showNotification({
              title: 'Error',
              message: payload.message || 'Failed to load crawler not match list',
              color: 'red',
            });
          }
        } else {
          showNotification({
            title: 'Error',
            message: response.error || 'Failed to load crawler not match list',
            color: 'red',
          });
        }
      } catch (error) {
        console.error('Crawler not match list fetch error:', error);
        showNotification({
          title: 'Error',
          message: 'Unable to load crawler not match list',
          color: 'red',
        });
      } finally {
        setLoading(false);
          ...
          >
            <Box>
              <Text
                ...
                fw={700}
              >
                Crawler List Not Match
              </Text>
              <Text
                size="sm"
                c="dimmed"
              >
                Non-matching crawler records
              </Text>
            </Box>
                ...
                <Table.Tbody>
                  {paginatedData.length > 0 ? (
                    paginatedData.map((item, idx) => (
                      <Table.Tr key={`${item.futuretrxid || item.trxid || 'row'}-${startIndex + idx}`}>
                        {visibleColumns.map((col) => (
...
export default AppiumListNotMatch;
