...
const CompanyAdjustment = () => {
  ...
  const validate = () => {
    if (!form.accountno) {
      showNotification({ title: 'Validation', message: 'Please choose an account', Color: 'yellow' });
      return false;
    }
    if (!form.type) {
  ...
  const handleSubmit = async () => {
    ...
    setLoading(true);
    try {
      const response = await transactionAPI.saveCompanyAdjustment(payload);
      if (response.success && response.data) {
      ...
      console.error('Save company adjustment error:', error);
...
export default CompanyAdjustment;
