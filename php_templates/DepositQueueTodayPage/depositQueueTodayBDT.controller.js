app.controller("depositQueueTodayBDTCtrl", [
  "$state",
  "$scope",
  "$http",
  "$timeout",
  ...
  function (
    ...
    $scope.pagelang = {
      pagetitle: "Deposit Queue Today BDT",
    };
    ...
    $scope.gridOptions = {
      ...
      columnDefs: [
        ...
        { name: "Flag", field: "flag", width: 100 },
        {
          ...
          width: 360,
          cellTemplate:
            '<button type="button" class="btn btn-primary btn-sm" style="margin-right:2px;" ng-click="grid.appScope.checkDuplicate(row.entity)" ng-show="row.entity.status == \'Order need to check\'"  >Check</button>' +
    ...
    $scope.checkDuplicate = function (params) {
      var modalInstance = $uibModal.open({
        animation: true,
        templateUrl:
          "js/Modal/CheckDuplicateModal/CheckDuplicateModal.template.html?v=" +
          new Date().getTime(),
    ...
    $scope.getListData = function () {
      ...
      $http({
        method: "POST",
        url: webservicesUrl + "/depositQueue_getTransactionTodayBdt.php",
        data: { data: jsonData },
        headers: {
          "Content-Type": "application/x-www-form-urlencoded;charset=UTF-8",
        },
