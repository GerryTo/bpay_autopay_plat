app.controller("smsLogByBalanceDiffCtrl", [
  "$state",
  "$scope",
  "$http",
  "$timeout",
  ...
  function (
    ...
    $scope.gridOptions = {
      ...
      exporterExcelFilename: "sms-log-by-balance-diff.xlsx",
    ...
    $scope.getListData = function () {
      if ($scope.filter.type == "") {
        alert("Please select balance diff criteria");
        return false;
      }
      ...
      var data = { type: $scope.filter.type };
      var jsonData = data;

      $http({
        method: "POST",
        url: webservicesUrl + "/smsLog_getByBalanceDiff.php",
        data: { data: jsonData },
        headers: {
        ...
        function mySuccess(response) {
          $scope.gridIsLoading = false;
          var data_response = response.data.data;
