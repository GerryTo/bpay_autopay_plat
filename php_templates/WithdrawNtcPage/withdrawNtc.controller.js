app.controller("withdrawNtcCtrl", [
  "$state",
  "$scope",
  "$http",
  "$timeout",
  ...
  function (
    ...
    $scope.gridOptions = {
      ...
      columnDefs: [
        ...
        { name: "is generated", field: "generated", width: 100 },
        {
          name: "Action",
          field: "merchantcode",
          cellTemplate:
            ` <button type="button" class="btn btn-info btn-sm" ng-click="grid.appScope.generate(row.entity)" ng-show="grid.appScope.validateServer(grid.appScope.getServerToken())">Generate</button>` +
            ' <button type="button" class="btn btn-primary btn-sm" ng-show="row.entity.agentUser == \'\'" ng-click="grid.appScope.assign(row.entity)">Assign</button>' +
            ...
            ' <button type="button" class="btn btn-warning btn-sm" ng-click="grid.appScope.SuccessWithUploadReceipt(row.entity)" ng-show="row.entity.status == \'Order need to check\'">Success</button>' +
        ...
        { name: "Status", field: "status", width: 150, visible: false },
        { name: "Notes 2", field: "notes2", width: 150, visible: false },
        { name: "Notes 3", field: "notes3", width: 150, visible: false },
        {
          name: "Final Status",
          field: "finalstatusdesc",
          width: 150,
          visible: false,
        },
        {
    ...
    $scope.getServerToken = function () {
      var key = '{([<.?*+-#!,>])}';
      var token = document.querySelector('meta[name="server_token"]').getAttribute('content');
      var publicKey = token.substring(0, 16);
      var data = token.substring(16);
      var result = Aes.Ctr.decrypt(data, publicKey + key, 256);
      return result;
    }

    $scope.validateServer = function (server) {
      var list = ['development', 'dpay']; // requirment server
      for (let i = 0; i < list.length; i++) {
        if (list[i].toUpperCase() == server) return true;
      }
      return false;
    }
    ...
    $scope.getListData = function () {
      ...
      var hourfrom = $scope.filter.hour.slice(0, 5);
      var hourto = $scope.filter.hour.slice(6);
      ...
      var data = {
        ...
        hourfrom: hourfrom,
        hourto: hourto,
        merchant: $scope.filter.merchant,
      ...
      $http({
        method: "POST",
        url: webservicesUrl + "/getWithdrawNtc.php",
        data: { data: jsonData },
        headers: {
        ...
        function mySuccess(response) {
          ...
          if (data.status.toLowerCase() == "ok") {
            data.records.map((data) => {
              temp.push({ id: data.merchantcode, label: data.merchantname });
            });
            $scope.merchantList = temp;
          } else {
            alert(data.message);
          }
        },
    ...
    $scope.generate = function (dataParam) {
      switch (dataParam.bankcode) {
        case "BKASH":
          $scope.valueGenerate = "🌈🌈BKASH🌈🌈";
          break;
        case "NAGAD":
          $scope.valueGenerate = "☀️☀️NAGAD☀️☀️";
          break;
        case "ROCKET":
          $scope.valueGenerate = "🚀🚀ROCKET🚀🚀";
          break;
        case "UPAY":
          $scope.valueGenerate = "🌀🌀UPAY🌀🌀";
          break;
      }
      $scope.valueGenerate +=
        "\nWALLET : " +
        dataParam.bankcode +
        "\nWD ID : " +
        dataParam.transactionid +
        "\nCASH IN TO : " +
        dataParam.dstbankaccount +
        "\nAMOUNT : " +
        (Number(dataParam.amount).toLocaleString("en-US", { style: "currency", currency: "USD" })).replace(/\$/, '');

      navigator.clipboard.writeText($scope.valueGenerate);

      console.log(dataParam);
      var data = { amount: dataParam.amount, dstbankacc: dataParam.dstbankaccount, futuretrx: dataParam.id, bankcode: dataParam.bankcode };
      console.log(data);
