app.controller("withdrawNtcFilterSelectedCtrl", [
  "$state",
  "$scope",
  "$http",
  "$timeout",
  ...
  function (
    ...
    $scope.submit = function () {
      ...
      for (var i = 1; i <= arr.length; i++) {
        if (tempBank == "") tempBank = arr[i - 1].bankcode;
        if (tempBank != arr[i - 1].bankcode) {
          diffBank = true;
          break;
        }
      }
      ...
      if (diffBank) {
        alert("Please only choose withdraw with same bank code.");
        return false;
      }
        ...
        function (returnValue) {
          ...
          function mySuccess(response) {
            for (var i = 0; i < response.length; i++) {
              // var data = CRYPTO.decrypt(response[i].data.data);
