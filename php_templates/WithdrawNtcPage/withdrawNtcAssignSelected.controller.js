app.controller("withdrawNtcAssignSelectedCtrl", [
  "$state",
  "$scope",
  "$http",
  "$timeout",
  ...
  function (
    ...
    $scope.gridOptions = {
      ...
      columnDefs: [
        ...
        { name: "TransactionID", field: "transactionid", width: 100 },
        {
          ...
          field: "merchantcode",
          cellTemplate:
            // ' <button type="button" class="btn btn-info btn-sm" ng-click="grid.appScope.generate(row.entity)">Generate</button>' +
    ...
    $scope.getListData = function () {
      ...
      var data = {
        ...
        bankCode: $scope.filter.bankCode,
      };
      //   console.log(data);
      //   return;
    ...
    $scope.submit = function () {
      ...
      if (arr.length == 0) {
        alert("Please choose withdraw to submit");
        return false;
      }
      ...
      var modalInstance = $uibModal.open({
        ...
        resolve: {
          params: function () {
            return { bankcode: $scope.filter.bankCode };
          },
        },
      });
      modalInstance.result.then(
        function (returnValue) {
          ...
          for (var i = 1; i <= arr.length; i++) {
            ...
            var promise = $http({
              method: "POST",
              url: webservicesUrl + "/withdrawAssignmentBulk.php",
              data: { data: jsonData },
              headers: {
                "Content-Type":
          ...
          Promise.all(promises).then(mySuccess).catch(myError);

          function mySuccess(response) {
            for (var i = 0; i < response.length; i++) {
              var data = response[i].data.data;
              if (data.status.toLowerCase() == "no") {
                alert(data.message);
              }
            }
            alert("Success!");
    ...
    $scope.getBankCode = function () {
      $http({
        ...
        function mySuccess(response) {
          ...
          if (data.status.toLowerCase() == "ok") {
            data.records.map((data) => {
              temp.push({ id: data.bankCode, label: data.bankCode });
            });
            $scope.bankList = temp;
          } else {
            alert(data.message);
