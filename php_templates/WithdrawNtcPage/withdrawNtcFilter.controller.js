app.controller("withdrawNtcFilterCtrl", [
  "$state",
  "$scope",
  "$http",
  "$timeout",
  ...
  function (
    ...
    $scope.getServerToken = function () {
      var key = "{([<.?*+-#!,>])}";
      var token = document
        .querySelector('meta[name="server_token"]')
    ...
    $scope.getListData = function () {
      ...
      $http({
        method: "POST",
        url: webservicesUrl + "/getwithdrawNtcFilter.php",
        data: { data: jsonData },
        headers: {
        ...
        function mySuccess(response) {
          ...
          if (data.status.toLowerCase() == "ok") {
            data.records.map((data) => {
              temp.push({ id: data.merchantcode, label: data.merchantcode });
            });
    ...
    $scope.generate = function (dataParam) {
      ...
      var data = {
        ...
        dstbankacc: dataParam.dstbankaccount,
        futuretrx: dataParam.id,
        bankcode: dataParam.bankcode,
      };
      console.log(data);
      ...
      $http({
        method: "POST",
        url: webservicesUrl + "/checkWithdrawNtcDuplicate.php",
        data: { data: jsonData },
        headers: {
          "Content-Type": "application/x-www-form-urlencoded;charset=UTF-8",
        },
