app.controller('transactionAccountByCompanyWithdrawCtrl', ['$state', '$scope', '$http', '$timeout', 'uiGridConstants', function ($state, $scope, $http, $timeout, uiGridConstants) {
	...
	enableGridMenu: true,
        ...
        columnDefs: [
            ...
            { name: 'Account Src', field: 'accountno' },
            { name: 'Merchant Code', field: 'merchantcode' },
            { name: 'Customer Code', field: 'customercode' },
            { name: 'CCY', field: 'ccy', visible:false },
            { name: 'Bank', field: 'sourcebankcode', sort: { direction: 'asc' }, filter: {condition: uiGridConstants.filter.EXACT } },
            { name: 'Credit', field: 'CR', cellFilter: 'number: '+decimalDigit, cellClass: 'grid-alignright', type:'number',
    ...
    $scope.getListData = function () {
        ...
        var data = { 'datefrom' : from, 'dateto': to, 'accountno': $scope.filter.accountno , transactiontype:'W' };
        var jsonData = CRYPTO.encrypt(data);
...
*/
                for(var i=0, length=data.records.length;i<length;i++) {
                        if( data.records[i]['transactiontype']=='D' || data.records[i]['transactiontype']=='N' || data.records[i]['transactiontype']=='Topup' || data.records[i]['transactiontype']=='Y' ) {
