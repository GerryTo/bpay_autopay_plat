app.controller('transactionByIdBackupNoActionCtrl', ['$state', '$scope', '$http', '$timeout', 'uiGridConstants', '$stateParams', '$uibModal', function ($state, $scope, $http, $timeout, uiGridConstants, $stateParams, $uibModal) {
    ...
    $scope.getListData = function () {
        ...
        $http({
            method: "POST",
            url: webservicesUrl + "/transactionByIdBackup_getList.php",
            data: { 'data': jsonData },
            headers: { 'Content-Type': 'application/x-www-form-urlencoded;charset=UTF-8' }
        }).then(function mySuccess(response) {
            $scope.gridIsLoading = false;
