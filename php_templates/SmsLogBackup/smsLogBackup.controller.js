app.controller('smsLogBackupCtrl', ['$state', '$scope', '$http', '$timeout', 'uiGridConstants','$uibModal', '$interval', function ($state, $scope, $http, $timeout, uiGridConstants, $uibModal, $interval ) {
    ...
    $scope.gridOptions = {
        ...
        columnDefs: [
            ...
            { name: 'Message', field: 'message', cellTooltip: 
                function( row, col ) {
            ...
            { name: 'Balance', field: 'balance', type: 'number', cellClass: 'grid-alignright', cellFilter: 'number: '+decimalDigit, type:'number', width:120 },
            { name: 'Balance Calculate', field: 'balancecalculate', type: 'number', cellClass: 'grid-alignright', cellFilter: 'number: '+decimalDigit, type:'number', width:120 },
            { name: 'Balance Different', field: 'balancediff', type: 'number', cellClass: 'grid-alignright', cellFilter: 'number: '+decimalDigit, type:'number', width:120 },
            // {
    ...
    $scope.getListData = function () {
        ...
        $http({
            method: "POST",
            url: webservicesUrl + "/smsLogBackup_getList.php",
            data: { 'data': jsonData },
    ...
    $scope.getPhoneUserList = function(){
        ...
        $http({
            method: "POST",
            url: webservicesUrl + "/smsLogBackup_getPhoneUserList.php",
            data: { 'data': jsonData },
            headers: { 'Content-Type': 'application/x-www-form-urlencoded;charset=UTF-8' }
        }).then(function mySuccess(response) {
            $scope.gridIsLoading = false;
    ...
    $scope.expire = function(dataParam){
        if(confirm('Are you sure want to expire this SMS?')){
            ...
            $http({
                method: "POST",
                url: webservicesUrl + "/smsLogBackup_expireSms.php",
                data: { 'data': jsonData },
                headers: { 'Content-Type': 'application/x-www-form-urlencoded;charset=UTF-8' }
            }).then(function mySuccess(response) {
                var data = CRYPTO.decrypt(response.data.data);
    ...
    $scope.match = function(dataParam){
        ...
        modalInstance.result.then(function (returnValue) {
            ...
            $http({
                method: "POST",
                url: webservicesUrl + "/smsLogBackup_saveMatchTransaction.php",
                data: { 'data': jsonData },
                headers: { 'Content-Type': 'application/x-www-form-urlencoded;charset=UTF-8' }
            }).then(function mySuccess(response) {
                var data = CRYPTO.decrypt(response.data.data);
