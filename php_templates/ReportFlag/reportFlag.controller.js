app.controller('reportFlagCtrl', ['$state', '$scope', '$http', '$timeout', 'uiGridConstants','$uibModal', '$interval', function ($state, $scope, $http, $timeout, uiGridConstants, $uibModal, $interval ) {
    ...
    $scope.gridOptions = {
        ...
        exporterExcelFilename: 'report-flag.xlsx',
        exporterExcelSheetName: 'Sheet1',
	    rowTemplate:'templates/rowTemplate.html',
        columnDefs: [
            { name: 'Merchant Code', field: 'merchantCode', width:220  },
            { name: 'Customer Code', field: 'customerCode', width:220  },
            { name: 'No. Different Src Acc', field: 'count', type: 'number', width:120, cellClass: 'grid-alignright'  },
            { name: 'Blacklist', field: 'isBlacklist', width:100  },
            {
                name: 'Action', field: 'customerCode',width:150, enableFiltering: false,
                cellTemplate: '<button type="button" class="btn btn-primary btn-sm" ng-click="grid.appScope.detail(row.entity)">Detail</button>'
    ...
    $scope.detail = function(params){
        var modalInstance = $uibModal.open({
            animation: true,
            templateUrl: 'js/Modal/FlagModal/FlagModal.template.html?v='+new Date().getTime(),
            controller: 'FlagModalCtrl',
            size: 'lg',
            scope: $scope,
            resolve:{
                items: function(){
                    return { merchantCode: params.merchantCode, customerCode: params.customerCode }
                }
            }
        });
    ...
    $scope.blacklist = function(params){
        if(confirm("Are you sure want to blacklist ["+params.merchantCode+", "+params.customerCode+"] ?")){
            var data = { 'merchantCode' : params.merchantCode, 'customerCode': params.customerCode };
    ...
    $scope.getListData = function () {
        ...
        $http({
            method: "POST",
            url: webservicesUrl + "/report_flagCustomer.php",
            data: { 'data': jsonData },
            headers: { 'Content-Type': 'application/x-www-form-urlencoded;charset=UTF-8' }
        }).then(function mySuccess(response) {
            $scope.gridIsLoading = false;
