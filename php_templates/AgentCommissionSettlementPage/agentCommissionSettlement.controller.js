app.controller('agentCommissionSettlementCtrl', ['$state', '$scope', '$http', '$timeout', 'uiGridConstants', '$stateParams', '$uibModal', 
    '$interval', '$rootScope', 
    ...
    $scope.gridOptions = {
        ...
        columnDefs: [
            { name: 'Settlement ID', field: 'id', aggregationType: uiGridConstants.aggregationTypes.count, footerCellTemplate: '<div class="ui-grid-cell-contents" style="text-align:right">{{col.getAggregationValue() }}</div>', 
              sort: {
            ...
            { name: 'Account Source', field: 'bankaccountsource' },
            { name: 'Account Target', field: 'bankaccounttarget' },
    ...
    $scope.approve = function(params){
        if(confirm('Are you sure want to approve this settlement?')){
            var params = { id : params.id };    // id : array()

            // var jsonData = CRYPTO.encrypt(params);
            $http({
                method: "POST",
                url: webservicesUrl + "/agent/cp_commission_settlement_approve.php",
                data: params,
    ...
    $scope.reject = function(params){
        ...
        let reason = prompt('Reject settlement. Please input the reason');
        ...
        if(reason != null){
            ...
            $http({
                method: "POST",
                url: webservicesUrl + "/agent/cp_commission_settlement_reject.php",
                data: params,
                headers: { 'Content-Type': 'application/x-www-form-urlencoded;charset=UTF-8' }
            }).then(function mySuccess(response) {
                // var data = CRYPTO.decrypt(response.data.data);
    ...
    $scope.getListData = function () {
        ...
        $http({
            method: "POST",
            url: webservicesUrl + "/agent/cp_get_commission_settlement_list.php",
            data: data,
            headers: { 'Content-Type': 'application/x-www-form-urlencoded;charset=UTF-8' }
        }).then(function mySuccess(response) {
            $scope.gridIsLoading = false;
