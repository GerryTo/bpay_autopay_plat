app.controller('smsFailedMatchByNotMatchSamedayCtrl', ['$state', '$scope', '$http', '$timeout', 'uiGridConstants','$uibModal', '$interval', function ($state, $scope, $http, $timeout, uiGridConstants, $uibModal, $interval ) {
  ...
  $scope.gridOptions = {
    ...
    rowTemplate:'templates/rowTemplate.html',
      columnDefs: [
          { name: 'Future Trx ID', field: 'futuretrxid', width:120, aggregationType: uiGridConstants.aggregationTypes.count,
              sort:{
          ...
          { name: 'Amount (SMS)', field: 'smsamount', type: 'number', cellClass: 'grid-alignright', cellFilter: 'number: '+decimalDigit, type:'number', width:120 },
          { name: 'Reason', field: 'reason', width:200 },
          
          {
              name: 'Action', field: 'securitycode', width:150, 
              cellTemplate: '<button type="button" class="btn btn-primary btn-sm"  ng-click="grid.appScope.match(row.entity)">Match</button> <button type="button" class="btn btn-warning btn-sm" ng-show="row.entity.futuretrxid == \'\'" ng-click="grid.appScope.expire(row.entity)">Expire</button>'
          },
  ...
  $scope.getListData = function () {
      ...
      $http({
          method: "POST",
          url: webservicesUrl + "/smsFailedMatchByNotMatchSameday_getList.php",
          data: { 'data': jsonData },
  ...
  $scope.match = function(dataParam){
      var modalInstance = $uibModal.open({
          ...
          resolve:{
              items: function(){
                  return { amount: dataParam.smsamount, bank: dataParam.smsbank, trxid: dataParam.trxid, phonenumber: dataParam.customerphone }
              }
          }
      });
