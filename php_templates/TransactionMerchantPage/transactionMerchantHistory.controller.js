app.controller('transactionMerchantHistoryCtrl', ['$state', '$scope', '$http', '$timeout', 'uiGridConstants', function ($state, $scope, $http, $timeout, uiGridConstants) {
    ...
    $scope.gridOptions = {
        ...
        enableGridMenu: true,
        exporterExcelFilename: 'MerchantTransactionHistory.xlsx',
	    ...
	    $scope.getCurrentBalanceNew= function () {
        ...
        $http({
            ...
            if (data.status.toLowerCase() == 'ok') {
		        $scope.MerchantBalanceNew =data.records[0];
		        console.log($scope.MerchantBalanceNew);
            } else {
                alert(data.message);
            }
    ...
    $scope.getListData = function () {
        ...
        $http({
            method: "POST",
            url: webservicesUrl + "/getTransactionByMerchantHistory.php",
            data: { 'data': jsonData },
            headers: { 'Content-Type': 'application/x-www-form-urlencoded;charset=UTF-8' }
        }).then(function mySuccess(response) {
            $scope.gridIsLoading = false;
